...
// Simplified in-memory store (remove sessionStorage dependency)
...
function ProductDetailPage() {
  ...
  const [totalPayments, setTotalPayments] = useState(0); // NEW: Store total payments
  ...
  useEffect(() => {
    fetchProductDetails();
    fetchTotalPayments(); // NEW: Fetch payments when filters change
  }, [showAll, dateRange, orderDate, receiveDate]);

  // NEW: Fetch total payments from backend
  const fetchTotalPayments = async () => {
    try {
      ...
      // Apply same date filters as product details
      ...
      const res = await request(`payments/total${queryString}`, 'get');

      if (res?.success) {
        setTotalPayments(parseFloat(res.totalAmount || 0));
      }
    } catch (error) {
  ...
  const grandTotals = useMemo(() => {
    const completed = filteredData.filter(item => item.is_completed).length;
    ...
    return {
      totalQuantity: Object.values(categoryTotals).reduce(
        (acc, category) => acc + category.totalQuantity,
        0
      ),
      totalValue: totalValue,
      remainingBalance: totalValue - totalPayments, // NEW: Calculate remaining balance
      totalItems: Object.values(categoryTotals).reduce(
        (acc, category) => acc + category.count,
        0
      ),
      completedItems: completed
    };
  }, [categoryTotals, filteredData, totalPayments]);
  ...
  const handleClearFilters = () => {
    ...
    setReceiveDate(null);
    fetchProductDetails();
    fetchTotalPayments(); // NEW: Also refresh payments
  };
            ...
            <Row gutter={16}>
              <Col span={6}>
                <Statistic
                  ...
                  value={grandTotals.totalQuantity}
              ...
              <Col span={6}>
                <Statistic
                  title={
                    <div>
                      <div className="khmer-text-product">តម្លៃទិញចូលសរុប</div>
              ...
              <Col span={6}>
                <Statistic
                  ...
                  value={grandTotals.remainingBalance}
                  precision={2}
                  prefix="$"
                  valueStyle={{ 
                    color: grandTotals.remainingBalance > 0 ? '#faad14' : '#52c41a' 
                  }}
                />
              </Col>
            </Row>
              ...
              <Row gutter={16} style={{ marginTop: 16 }}>
                <Col span={24}>
                  <Statistic
                    ...
                    value={grandTotals?.completedItems ?? 0}
                    suffix={`/ ${filteredData?.length ?? 0}`}
