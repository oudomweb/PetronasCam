...
//     // First pass: calculate actual total amount
//     for (const item of items) {
//       if (!item.category_id || !item.quantity || !item.unit_price) {
//         await connection.rollback();
//         return res.status(400).json({ error: "Each item must have category_id, quantity, and unit_price" });
...
//       calculatedTotalAmount += item_total;
//     }

//     // Use calculated total if total_amount is 0 or not provided
//     const finalTotalAmount = total_amount > 0 ? total_amount : calculatedTotalAmount;
...
//     // ✅ Calculate total liters
//     const totalLiters = items.reduce((sum, item) => sum + (item.quantity || 0), 0);
...
//     telegramText += `🏠 Address: <b>${customer.address}</b>\n`;
//     telegramText += `📞 Phone: <b>${customer.tel}</b>\n`;
//     telegramText += `💰 Total: <b>$${parseFloat(finalTotalAmount).toLocaleString()}</b>\n`;
//     telegramText += `📝 Created By: <b>${createdBy}</b>\n\n`;
...
//       ids: createdInvoices,
//       success: true,
//       total_amount: finalTotalAmount // Return the calculated total
//     });
...
exports.getList = async (req, res) => {
  try {
    ...
    res.status(500).json({
      error: "Failed to get invoice list",
      message: error.message,
      success: false
    });
  }
