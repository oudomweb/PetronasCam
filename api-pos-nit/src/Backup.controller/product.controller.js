...
exports.getProductDetail = async (req, res) => {
  try {
    ...
    // ✅ FIX: ប្រើ DATE() function ដើម្បីប្រៀបធៀបតែថ្ងៃប៉ុណ្ណោះ (មិនរាប់បញ្ចូលម៉ោង)
    // Priority: order_date > receive_date > date_range

    if (order_date) {
      // ✅ ប្រៀបធៀបតែផ្នែកថ្ងៃតែប៉ុណ្ណោះ
      sqlWhere += ` AND DATE(pd.created_at) = DATE(?)`;
      params.push(order_date);
      
      // 🔍 DEBUG: បង្ហាញ query ក្នុង console
      console.log('🔍 Order Date Filter:', order_date);
    } else if (receive_date) {
      sqlWhere += ` AND DATE(pd.receive_date) = DATE(?)`;
      params.push(receive_date);
      
      console.log('🔍 Receive Date Filter:', receive_date);
